...
public class UserServiceImpl implements UserService {
	...
	public List<Subtask> getUserSubtaskActivities(User user) {
		...
		System.out.println("role " + user.getUserRole().getRoleId());

		if (user.getUserRole().getRoleId() == 1 || user.getUserRole().getRoleId() == 2) {
			subtasks = userDao.getAllReviewSubtasks();
		} else {
			
			subtasks = userDao.getUserSubtaskActivities(user.getUserId());
